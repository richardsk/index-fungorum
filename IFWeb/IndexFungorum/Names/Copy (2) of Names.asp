...
<table width="100%" height="100%" class="mainbody">
  ...
  <tr>
    <td align="left" valign="top"><table width="100%" cellpadding="5">
        <tr>
          <td><!-- InstanceBeginEditable name="head" --> 
            <%
	dim strSearch, strType, intPageNumber, intNumRecords, intPageSize, intPageCount, strOpen, strBlank
	...
	if request.form.item("Submit") <> "" then
		if request.form.item("SearchTerm") <> ""  then
			...
			elseif request.form.item("SearchBy") = "Epithet" then
				strType = "E"
				session("strType") = strType
			elseif request.form.item("SearchBy") = "Genus" then
				strType = "G"
				session("strType") = strType
			elseif request.form.item("SearchBy") = "Family" then
				strType = "F"
				session("strType") = strType
			elseif request.form.item("SearchBy") = "Higher" then
				strType = "H"
				session("strType") = strType
			else
				strBlank = "true"
	...
	if request.querystring.item("strGenus") <> "" then
		...
		if request.querystring.item("GSD") <> "" then strType = "GSD"
'	end if
	elseif request.querystring.item("strEpithet") <> "" then
		...
		session("intPageNumber") = intPageNumber
'	end if
	elseif request.querystring.item("strFamily") <> "" then
		strSearch = request.querystring.item("strFamily")
		session("strSearch") = strSearch
		strType = "F"
		session("strType") = strType
		intPageNumber = "1"
...
sub ResList (rs)
	...
	' code for type of entry
	if StrType = "N" OR strType = "E" OR strType = "G" then
		response.write("<b>Name, Author, Year, (Current name), Parent taxon</b><br>")
	else
		response.write("<b>Name, Author, Year</b><br>")
	end if
	NavForm
	for i = 1 to rs.PageSize
		...
		if rs("RECORD NUMBER") = rs("CURRENT NAME RECORD NUMBER") then
			...
			if rs("YEAR OF PUBLICATION") <> "" then response.write(" " & rs("YEAR OF PUBLICATION"))
			strLinkType = 4
			strLink = "http://www.speciesfungorum.org/Names/SynSpecies.asp?RecordID=" & rs("CURRENT NAME RECORD NUMBER")
			response.write(", (also see Species Fungorum: ")
	...
	' ADD CODE FOR HIGHER RANKS
	if StrType = "N" OR strType = "E" then
		...
		' there is no current name so either get parent from name or if current name fundic record number included get from same as above
			...
			' there is no current name so get parent from name
	...
	elseif StrType = "G" then
		if right(rs("family name"),5) = "aceae" then
			if parenttaxon = "" then
				parenttaxon = "<a href='Names.asp?strFamily=" & rs("family name") & "'>" & rs("family name") & "</a>"
			end if
		end if
		if right(rs("order name"),4) = "ales" then
			if parenttaxon = "" then
	...
	'Code here for suprageneric results if wanted
...
sub OrthVar
	   ...
	   dbConn.open "FILEDSN=cabilink"
		...
		if strType = "N" then
			strSQL = "SELECT IndexFungorum.* FROM IndexFungorum " _
				& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearch & "%') AND ((IndexFungorum.[STS FLAG])='o') " _
		...
		elseif strType = "E" then
			strSQL = "SELECT IndexFungorum.* FROM IndexFungorum " _
				& "WHERE (((IndexFungorum.[SPECIFIC EPITHET]) Like '" & strSearch & "%') AND ((IndexFungorum.[STS FLAG])= 'o')) " _
				& "OR (((IndexFungorum.[INFRASPECIFIC EPITHET]) Like '" & strSearch & "%') AND ((IndexFungorum.[STS FLAG])= 'o')) " _
	...
	if not rs.bof then
		...
		response.write("<br>No records found; use the 'add new record' link above")		
	end if
	rs.close
	set rs = nothing
	dbConn.close
...
sub DisplayRes
	...
	if strType = "N" then
		strSQL = "SELECT TOP 6000 IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.AUTHORS, " _
			...
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearch & "%')) " _
	...
	elseif strType = "E" then
		strSQL = "SELECT TOP 6000 IndexFungorum.[SPECIFIC EPITHET], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.[NAME OF FUNGUS], " _
			...
			if strOpen <> "OK" then
				strSQL = strSql & "WHERE (((IndexFungorum.[SPECIFIC EPITHET]) Like '" & strSearch & "%') " _
					& "AND ((IndexFungorum.[STS FLAG]) Is Null)) " _
					& "OR (((IndexFungorum.[INFRASPECIFIC EPITHET]) Like '" & strSearch & "%') AND ((IndexFungorum.[STS FLAG]) Is Null)) " _
					& "ORDER BY IndexFungorum.[SPECIFIC EPITHET], IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[YEAR OF PUBLICATION];"
			else
				strSQL = strSql & "WHERE (((IndexFungorum.[SPECIFIC EPITHET]) Like '" & strSearch & "%')) " _
					& "OR (((IndexFungorum.[INFRASPECIFIC EPITHET]) Like '" & strSearch & "%')) " _
					& "ORDER BY IndexFungorum.[SPECIFIC EPITHET], IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[YEAR OF PUBLICATION];"
			end if
	elseif strType = "G" then
		strSQL = "SELECT IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.AUTHORS, " _
			...
			& "ON IndexFungorum.[CURRENT NAME FUNDIC RECORD NUMBER] = FundicClassification_1.[Fundic Record Number] " _
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearch & "%')) " _
			& "AND (((IndexFungorum.[STS FLAG]) = 'g')) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[YEAR OF PUBLICATION];"
   	elseif strType = "F" then
		strSQL = "SELECT IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.AUTHORS, " _
			...
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearch & "%')) " _
			& "AND (((IndexFungorum.[STS FLAG]) = 'f')) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[YEAR OF PUBLICATION];"
   	elseif strType = "H" then
		strSQL = "SELECT IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.AUTHORS, " _
			...
			& "FROM IndexFungorum " _
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearch & "%') " _
			& "AND ((IndexFungorum.[STS FLAG]) = 'r' Or (IndexFungorum.[STS FLAG]) = 'c' Or (IndexFungorum.[STS FLAG]) = 'p')) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[YEAR OF PUBLICATION];"
  else
	exit sub
  end if
...
'		response.write("No Records Found So Trying Orthographic Variants")		
		OrthVar
	end if
end sub
%>
            ...
            <form method="post" action="Names.asp" name="search">
              <table width="651" border="0" name="Names" height="60">
                <tr> 
                  <td colspan="6"><b>Search by:-</b></td>
                  <td colspan="4" align="right"><a href="IndexFungorumRegister.htm"><img src="../IMAGES/new.gif" alt="register and publish names with a few mouse clicks" width="32" height="16" border="0"></a> 
<%
...
%> </td>
                ...
                <tr> 
                  ...
                  <td><b>higher</b></td>
                  <td colspan="2">&nbsp;</td>
                  <td width="144"><b>Enter a search term:-</b></td>
                  <td width="124">&nbsp;</td>
                  <td align="right"><a class='LinkColour1a' href="IndexFungorumAdd.asp">add 
                    new record</a></td>
                </tr>
                <tr> 
                  <td width="38" align="left" valign="top"> 
                    <input type="radio" name="SearchBy" value="Name" <%if strType <> "E" or strType <> "G" or strType <> "F" or strType <> "H" then response.write("checked")%>>
                  </td>
                  ...
                  <td width="41" align="left" valign="top"> 
                    <input type="radio" name="SearchBy" value="Genus" <%if strType = "G" then response.write("checked")%>>
                  </td>
                  <td width="44" align="left" valign="top"> 
                    <input type="radio" name="SearchBy" value="Family" <%if strType = "F" then response.write("checked")%>>
                  </td>
                  <td width="42" align="left" valign="top"> 
                    <input type="radio" name="SearchBy" value="Higher" <%if strType = "H" then response.write("checked")%>>
                  </td>
                  <td colspan="2">&nbsp;</td>
                  <td colspan="2" valign="top">
	<input type="text" name="SearchTerm" size="45" <%response.write("value=" & chr(34) & strSearch & chr(34))%>
