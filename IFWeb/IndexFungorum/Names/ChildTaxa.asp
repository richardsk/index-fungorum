...
dim strSQL, dbConn, rs, strSQL2, rs2
	...
	if rs.recordcount > 0 then
		...
		if strRank = "family" then
			...
			response.write("<b>" & rs("CURRENT NAME") & "</b> " & server.HTMLEncode(rs("AUTHORS")) & " " & rs("YEAR OF PUBLICATION") & "<br>")
			...
			if rs2.recordcount > 0 then
				do while not rs2.eof or rs2.bof
						response.write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;=  " & rs2("NAME OF FUNGUS") & " " & server.HTMLEncode(rs2("AUTHORS")) & " " & rs2("YEAR OF PUBLICATION") & "<br>")
				rs2.movenext
				loop
			end if
		else
			response.write("<b>" & rs("NAME OF FUNGUS") & "</b> " & server.HTMLEncode(rs("AUTHORS")) & " " & rs("YEAR OF PUBLICATION") & "<br>")
		end if
		rs.movenext
		loop
	else
