...
sub DisplayLinkedData(rs, strShow)
	...
	if strShow then
		...
		if strData <> "" then
' code to add link to homonym or invalid name
			if instr(strData,"$") then
				strData2 = left(strData,instr(strData,"$"))
				strData = right(strData,len(strData)-instr(strData,"$"))
			end if
			strData2 = left(strn(rs("NOMENCLATURAL COMMENT")), 13)
				if instr(strData2,"illegit") then
					response.write("<p><b>Editorial comment: </b><br>A homonym of " & strData & "</p>")
				elseif instr(strData2,"inval") then
					response.write("<p><b>Editorial comment: </b><br>See " & strData & "</p>")
				else
