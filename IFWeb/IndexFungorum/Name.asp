...
<title>HerbIMI-Online - name search</title>
<!-- InstanceEndEditable -->
...
if strIP = "10.0.5.4" then
	if strSearch <> "" then
		   strSQL = "SELECT DISTINCT tblHerbIMI.FinalNameDataEdited " _
				& "FROM tblHerbIMI " _
				& "WHERE (((tblHerbIMI.FinalNameDataEdited) Like '" & protectSQL(strSearch, false) & "%')) " _
				& "ORDER BY tblHerbIMI.FinalNameDataEdited;"
	else
	exit sub
	end if
else
	if strSearch <> "" then
		   strSQL = "SELECT DISTINCT tblHerbIMI.NameOfFungus " _
				& "FROM tblHerbIMI " _
				& "WHERE (((tblHerbIMI.NameOfFungus) Like '" & protectSQL(strSearch, false) & "%')) " _
				& "ORDER BY tblHerbIMI.NameOfFungus;"
	else
	exit sub
	end if
end if
...
sub ResList(RS)
	dim strLink, strOut
		response.write("<p></p><b>Name of Fungus: " & strn(rs("FinalNameDataEdited")) & "</b><p></p>") 
		do while not RS.bof and not RS.eof
		strText = ""
		strText = strn(rs("FinalNameDataEdited"))
		if strText <> "" then 
		   strLink = "DisplayResults.asp?strName=" & server.urlencode(rs("FinalNameDataEdited")) 
...
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Main" -->
            <form method="post" action="Name.asp" name="search">
              <table width="293" border="1" name="Name" height="60" bgcolor="#99FF99" bordercolor="#000000">
                <tr bgcolor="#99FF99" bordercolor="#99FF99"> 
                  <td width="133" height="15" bordercolor="#99FF99"><b><font face="Verdana, Arial, Helvetica, sans-serif">Name 
                    of Fungus:</font></b></td>
                ...
                <tr bgcolor="#99FF99" bordercolor="#99FF99"> 
                  ...
                  <td width="140" height="3"><input type="submit" name="submit" value="Search for fungus">
				  </td>
                </tr>
              </table>
			  </form>
