...
elseif strType = "1" then
	...
	if right(strSearchName,1) = "*" then strSearchName = left(strSearchName,len(strSearchName)-1)
...
elseif strType = "5" then
	...
	elseif strType = "2" then
		strSQL = "SELECT IndexFungorum.*, FundicClassification.[Family name], Publications.pubIMIAbbr, " _
			...
			& "WHERE (((IndexFungorum.[GSD FLAG]) Like 'GSD%') AND ((IndexFungorum.[CURRENT NAME RECORD NUMBER]) = " & intTaxonID & ")) " _
			& "OR (((IndexFungorum.[CURRENT NAME RECORD NUMBER]) = " & intTaxonID & ") AND ((IndexFungorum.[TAXONOMIC REFEREE]) Is Not Null)) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"
			...
			if not RS.bof then
				...
				do while not RS.eof
					...
					if RS("CURRENT NAME RECORD NUMBER") = RS("RECORD NUMBER") then
						...
						response.write(strText)
'						strText = chr(60) & "LITREF AUTHOR=" & chr(34) & "anon." & chr(34) & " TITLE=" & chr(34) & "unavailable" & chr(34) & " YEAR=" 
				...
				do while not RS.eof
					if RS("CURRENT NAME RECORD NUMBER") <> RS("RECORD NUMBER") then
						...
						if RS("INFRASPECIFIC EPITHET") <> "" then
							strText = strText & " " & "INFRASPECIFICPORTION=" & chr(34) & RS("INFRASPECIFIC RANK") & " " & RS("INFRASPECIFIC EPITHET") & chr(34)
						end if
