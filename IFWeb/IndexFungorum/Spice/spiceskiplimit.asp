...
<%
dim strType, strSearchName, intTaxonID, strTaxon, strRank, strText, strSQL, dbConn, RS, strTruncation, intLimit, intSkip
...
elseif strType = "1" then
	strSearchName = request.querystring.item("searchname")
		...
		strSearchName = replace(strSearchName,"%","")
	intLimit = request.querystring.item("searchname")
	intSkip = request.querystring.item("searchname")
...
'			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearchName & "%') AND ((IndexFungorum.[GSD FLAG]) Like 'GSD%')) " _
'			& "OR (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearchName & "%') AND ((IndexFungorum.[TAXONOMIC REFEREE]) Is Not Null)) " _
'			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"

intLimit = 10
intSkip = 20

		strSQL = "SELECT * FROM [SELECT TOP 10 * FROM (SELECT TOP 30 * FROM IndexFungorum ORDER BY [IndexFungorum].[NAME OF FUNGUS] ASC) AS Alias1 ORDER BY [IndexFungorum].[NAME OF FUNGUS] DESC]. AS Alias2 ORDER BY [IndexFungorum].[NAME OF FUNGUS];"




'		strSQL = "SELECT * " _
'			& "FROM [SELECT TOP" & intLimit & "* " _
'			& "FROM (SELECT TOP " & intSkip & " IndexFungorum.*, IndexFungorum_1.AUTHORS AS [CURRENT NAME AUTHORS], " _
'			& "IndexFungorum_1.[SPECIFIC EPITHET] AS [CURRENT NAME SPECIFIC EPITHET] " _
'			& "FROM IndexFungorum INNER JOIN " _
'			& "IndexFungorum IndexFungorum_1 ON " _
'			& "IndexFungorum.[CURRENT NAME RECORD NUMBER] = IndexFungorum_1.[RECORD NUMBER] " _
'			& "WHERE (IndexFungorum.[NAME OF FUNGUS] LIKE '" & strSearchName & "%') AND (IndexFungorum.[GSD FLAG] LIKE 'GSD%') " _
'			& "OR (IndexFungorum.[NAME OF FUNGUS] LIKE '" & strSearchName & "%') AND (IndexFungorum.[TAXONOMIC REFEREE] IS NOT NULL) " _
'			& "ORDER BY IndexFungorum.[NAME OF FUNGUS] ASC) AS ALIAS1 " _
'			& "ORDER BY [NAME OF FUNGUS] DESC]. AS ALIAS2 " _
'			& "ORDER BY [NAME OF FUNGUS];"
	...
	elseif strType = "2" then
		strSQL = "SELECT IndexFungorum.*, FundicClassification.[Family name], Publications.pubIMIAbbr, " _
			...
			& "WHERE (((IndexFungorum.[GSD FLAG]) Like 'GSD%') AND ((IndexFungorum.[CURRENT NAME RECORD NUMBER]) = " & protectSQL(intTaxonID,true) & ")) " _
			& "OR (((IndexFungorum.[CURRENT NAME RECORD NUMBER]) = " & protectSQL(intTaxonID,true) & ") AND ((IndexFungorum.[TAXONOMIC REFEREE]) Is Not Null)) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"
...
'TYPE 2 RESPONSE
		elseif strType = "2" then
			...
			if not RS.bof then
				...
				do while not RS.eof
					'run through the record set to find the AVC to get the standard data and output the AVC record
				...
				RS.Movefirst
				'start at the first record again to output all the synonym records
				do while not RS.eof
					if RS("CURRENT NAME RECORD NUMBER") <> RS("RECORD NUMBER") then
