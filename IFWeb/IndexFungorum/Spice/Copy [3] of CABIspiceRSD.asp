...
elseif strType = "1" then
	strSearchName = request.querystring.item("searchname")
		...
		if len(strSearchName) = 1 and strSearchName = "*" or strSearchName = "%" then
			strType = "1a"
			...
			if intLimit = "-1" then
				...
				if intSkip = "0" then
					...
					' trap for null Skip NOT QUITE RIGHT YET ;-)
					if intSkip = "" then
						strTruncation = "true"
					else
		...
		elseif len(strSearchName) > 1 and left(strSearchName,1) = "*" or left(strSearchName,1) = "%" then 
			strTruncation = "true"
		else
...
elseif strType = "5" then
	...
	elseif strType = "2" then
' simple query to test simple response
		strSQL = "SELECT IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[RECORD NUMBER] " _
			& "FROM IndexFungorum " _
			& "WHERE (((IndexFungorum.[CURRENT NAME RECORD NUMBER]) = " & intTaxonID & ")) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"
...
'						strText = strText & left(RS("TAXONOMIC REFEREE"),instr(RS("TAXONOMIC REFEREE"),"(")-2) & chr(34)
					...
					'determine if there is more than one taxonomic referee
	'						if RS("TAXONOMIC REFEREE") <> ""
	'							if instr(RS("TAXONOMIC REFEREE"),";") then
	'								strText1 = left(RS("TAXONOMIC REFEREE"),instr(RS("TAXONOMIC REFEREE"),";"))
	'								if instr(strText1,"(19") or instr(strText1,"(20;") then strText = strText & left(strText1,instr(strText1,"(")-2) & chr(34)
	'							else
	'								if instr(RS("TAXONOMIC REFEREE"),"(19") or instr(RS("TAXONOMIC REFEREE"),"(20;") then strText = strText & left(RS("TAXONOMIC REFEREE"),instr(RS("TAXONOMIC REFEREE"),"(")-2) & chr(34)
	'							end if
	'						else
	'							strText = strText & chr(34)
	'						end if
