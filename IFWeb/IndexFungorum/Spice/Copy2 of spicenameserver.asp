...
'			& "WHERE ((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearchName & "%') AND ((IndexFungorum.[INFRASPECIFIC EPITHET]) Is Null) AND ((IndexFungorum.[MISAPPLICATION AUTHORS]) Is Null) " _
'			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"
	...
	elseif strType = "1" then
		strSQL = "SELECT IndexFungorum.*, IndexFungorum_1.AUTHORS AS [CURRENT NAME AUTHORS], " _
			...
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & strSearchName & "%')) " _
			& "ORDER BY IndexFungorum.[NAME OF FUNGUS];"
...
'TYPE 3 RESPONSE
		elseif strType = "3" then
			...
			strText = strText & " DESCRIPTION=" & chr(34) & "Species Fungorum - global nomenclator for fungi" & chr(34) 
