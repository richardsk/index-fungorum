...
' CABI firewall
	if strA = "194" and strB = "131" and strC = "255" and strD = "2" then UserOK = "OK"
' Kirk_net address
...
dim i, strOut, strText, strUnlock, strPass
	...
	Displaygenera rs("article")
end sub

function DisplayGenera(intArticle)
	dim strSQL, dbConn, rs
	if intArticle <> ""  then 
		strSQL = "SELECT * FROM ArticleGenera " _
				& "WHERE (((article)=" & protectSQL(intArticle,true) & ")) " _
				& "order by genusname;"
	else
	...
	if not rs.eof and not rs.bof then
		response.write("<p><b>Genera:</b><br>")
	end if
	do while not rs.eof
		response.write(strn(rs("genusname")) & "<br>")
		rs.movenext
	loop
	response.write("</p>")
	rs.close
...
<!-- #EndEditable --><!-- #BeginEditable "Main" -->
      <h1>Bibliography of Systematic Mycology Record</h1>
<%
	DisplayRes
%>
      <!-- #EndEditable --></td>
