...
elseif strType = "5" then
	...
	elseif strType = "1" then
		strSQL = "SELECT IndexFungorum.*, IndexFungorum_1.AUTHORS AS [CURRENT NAME AUTHORS], " _
			...
			& "WHERE (((IndexFungorum.[NAME OF FUNGUS]) Like '" & protectSQL(strSearchName, false) & "%') AND ((IndexFungorum.[GSD FLAG]) = 'RSD')) " _
	...
	elseif strType = "1a" then
		strSQL = "SELECT IndexFungorum.*, tblSkipLimit.ID, IndexFungorum_1.AUTHORS AS [CURRENT NAME AUTHORS], " _
			...
			& "AND ((tblSkipLimit.ID)>" & protectSQL(intSkip,true) & " And (tblSkipLimit.ID)<" & protectSQL(intLimit,true) & ")) " _
...
'TYPE 3 RESPONSE
	elseif strType = "3" then
		...
		strText = strText & " DESCRIPTION=" & chr(34) & "The Index Fungorum (maintained by the Index Fungorum Partnership) is the global nomenclatural database for names of fungi at species rank and below; the Species Fungorum component recognizes a subset as the correct name for a taxon or as a synonym" & chr(34)
