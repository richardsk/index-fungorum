...
sub DisplayRes
	...
	case "G"
		...
		& "WHERE (((FundicClassification.[Genus name]) Like '" & strGenus & "%') AND ((IndexFungorum.[GSD FLAG]) Like 'GSD%')) " _
	...
	case "O"
		...
		& "FROM IndexFungorum INNER JOIN FundicClassification " _
		& "ON IndexFungorum.[CURRENT NAME FUNDIC RECORD NUMBER] = FundicClassification.[Fundic Record Number] " _
		& "WHERE (((FundicClassification.[Order name])='" & strOrder & "') AND ((IndexFungorum.[GSD FLAG]) Like 'GSD%')) " _
	...
	case "F"
		...
		if strOrder <> "not assigned" then
			strSql = strSql & "(((FundicClassification.[Order name])='" & strOrder & "')) and "
		else
			strSql = strSql & "((FundicClassification.[Order name]) is null) and "
		end if
		if strFamily <> "not assigned" then
			strSql = strSQL & "(((FundicClassification.[Family name])='" & strFamily & "'))"
		else
