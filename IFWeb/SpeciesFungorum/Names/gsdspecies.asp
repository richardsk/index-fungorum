...
' find name data - assumes strRecordID is a pointer to the current name record
   strSQL ="SELECT Publications.*, IndexFungorum.*, " _
		...
		& "WHERE ((IndexFungorum.[RECORD NUMBER])=" & protectSQL(strRecordID,true) & ") AND ((IndexFungorum.[STS FLAG]) Is Null);"
	...
	if strn(rs("SPECIFIC EPITHET")) = strn(rs("INFRASPECIFIC EPITHET")) then
		...
		if instr(strName," sensu ") then
			...
			if instr(strName,"; fide") then
				DisplayName = "<a class='LinkColour" & strLinkType & "' href='../Names/NamesRecord.asp?RecordID=" & rs("Record Number") & "'>" & left(strName,instr(strName,"; ")-1)  & "</a></b>" & right(strName,len(strName)-instr(strName,"; ")+1)
				...
				DisplayName = "<a class='LinkColour" & strLinkType & "' href='../Names/NamesRecord.asp?RecordID=" & rs("Record Number") & "'>" & strName  & "</a>"
			...
			DisplayName = "<a href='../Names/NamesRecord?RecordID=" & rs("Record Number") & "'>" & strName  & "</a>"
			if strName <> "" and right(strName,1) <> "," and strLit <> "" then DisplayName = DisplayName & ","
			DisplayName = DisplayName & strLit
		end if
		...
		if instr(strName," sensu ") then
			...
			DisplayName = "<a href='../Names/NamesRecord.asp?RecordID=" & rs("Record Number") & "'>" & strName  & "</a>"
			if strName <> "" and right(strName,1) <> "," and strLit <> "" then DisplayName = DisplayName & ","
			DisplayName = DisplayName & strLit
		end if
	end if
