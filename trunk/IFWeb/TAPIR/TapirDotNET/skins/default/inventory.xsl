...
<xsl:template match="/">
  ...
  <xsl:variable name="base-url" select="substring-before($accesspoint, 'tapir.aspx')" />
  <xsl:variable name="skin" select="tapir:response/tapir:header/tapir:custom/tapir:skin" />
  <html>
  <head>
  ...
  <body>
    ...
    <table border="0" cellspacing="1" cellpadding="3" bgcolor="#999999">
      ...
      <tr bgcolor="#FFFFFF">
        <td align="left"><a href="{$accesspoint}?op=s&amp;xslt={$base-url}skins/{$skin}/search.xsl&amp;s=0&amp;l=20&amp;cnt=1&amp;e=1&amp;f={$concept-id}%20equals%20%22{tapir:value}%22&amp;m={$base-url}model.xml.aspx%3Fc%3D{$concept-id}%26v%3D{tapir:value}%26a%3D{$accesspoint}"><xsl:value-of select="tapir:value"/></a></td>
