...
<title>Suprafamilial Names Record Details</title>
<!-- #EndEditable -->
    ...
    <td align="left" valign="top"><table width="100%" cellpadding="5">
        <tr>
          <td bgcolor="#CCFFCC"><!-- #BeginEditable "head" --><!-- #EndEditable --><!-- #BeginEditable "Main" --> 
            <h2>Suprafamilial Names Record Details </h2>
<%
...
%> 
      <p>Please contact <a href="mailto:p.kirk@cabi.org">Paul Kirk</a> if you have any additions or errors.</p>
<%
...
sub DisplayResults()
   dim strSQL, dbConn, rs, strName
	...
	strSQL = "SELECT [tblSupraFamilialTaxa].* "_
		& "FROM [tblSupraFamilialTaxa] " _
		& "WHERE ((([tblSupraFamilialTaxa].[ID]) =" & protectSQL(request.querystring.item("strRecordID"),true) & "));"
...
sub DisplayLinkedData(rs)
	...
	if strData <> "" then
		response.write("<p><b>Comments:</b><br> " & strData & "</p>")
	end if
end sub
