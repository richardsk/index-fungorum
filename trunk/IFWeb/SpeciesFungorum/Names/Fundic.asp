...
dim strSQL, dbConn, RS
   ...
   select case strType
       ...
       strSQL = "SELECT [FundicClassification].[Genus name], " _
			...
			& "WHERE ((([FundicClassification].[Genus name]) Like '" & protectSQL(strSearch, false) & "%') " _
			& "AND ((FundicClassification.[Current Use])='X')) " _
...
'			& "ORDER BY [Kingdom name]; "
     ...
     case "KS" 
       strSQL = "SELECT DISTINCT [Kingdom name] " _
	 		& "FROM [FundicClassification] " _
			& "WHERE ((([Kingdom name])='Fungi' OR ([Kingdom name])='Chromista' OR ([Kingdom name])='Protozoa'))" _
			& "ORDER BY [Kingdom name]; "
      ...
      case "K"
        strSql = "SELECT DISTINCT [Kingdom name], " _
	 		& "[Phylum name] " _
			& "FROM [FundicClassification] " _
			& "WHERE ((([Kingdom name])='" & protectSQL(strKingdom, false) & "')) " _
	  ...
	  case "P"
        strSql = "SELECT DISTINCT [Kingdom name], " _
			...
			& "[Class name] " _
			& "FROM [FundicClassification] " _
			& "WHERE ((([Kingdom name])='" & protectSQL(strKingdom, false) & "')) and "
			if strPhylum <> "not assigned" then
			   strSql = strSQL & "((([Phylum name])='" & protectSQL(strPhylum, false) & "')) "
			else
        ...
        strSql = "SELECT DISTINCT [Kingdom name], " _
			...
			& "WHERE ((([Kingdom name])='" & protectSQL(strKingdom, false) & "')) and "
			if strPhylum <> "not assigned" then
			   strSql = strSQL & "((([Phylum name])='" & protectSQL(strPhylum, false) & "')) and "
			else
			   strSql = strSql & "(([Phylum name]) is null) and "
			end if
			if strClass <> "not assigned" then
			   strSql = strSql & "((([Class name])='" & protectSQL(strClass, false) & "'))"
			else
        ...
        strSql = "SELECT DISTINCT [Kingdom name], " _
			...
			if strClass <> "not assigned" then
			   strSql = strSQL & "((([Class name])='" & protectSQL(strClass, false) & "')) and "
			else
			   strSql = strSql & "(([Class name]) is null) and "
			end if
			if strSubClass <> "not assigned" then
			   strSql = strSQL & "((([Subclass name])='" & protectSQL(strSubClass, false) & "')) "
			else
        ...
        strSql = "SELECT DISTINCT [Kingdom name], " _
			...
			if strSubClass <> "not assigned" then
			   strSql = strSQL & "((([Subclass name])='" & protectSQL(strSubClass, false) & "')) and "
			else
			   strSql = strSql & "(([Subclass name]) is null) and "
			end if
			if strOrder <> "not assigned" then
			   strSql = strSQL & "((([Order name])='" & protectSQL(strOrder, false) & "')) "
			else
        ...
        strSql = "SELECT [Kingdom name], " _
			...
			if strOrder <> "not assigned" then
			   strSql = strSQL & "((([Order name])='" & protectSQL(strOrder, false) & "')) and "
			else
			   strSql = strSql & "(([Order name]) is null) and "
			end if
			if strFamily <> "not assigned" then
			   strSql = strSQL & "((([Family name])='" & protectSQL(strFamily, false) & "'))"
			else
