...
sub DisplayResults()
   ...
   strSQL ="SELECT IndexFungorum.*, Publications.*, [FundicClassification].[family name], [FundicClassification].[order name], " _
		  & "[FundicClassification].[subclass name], [FundicClassification].[class name], [FundicClassification].[subphylum name], " _
  ...
  strShow = "False"
	if not (instr(strn(rs("LAST FIVE YEARS FLAG")),"X") > 0) and not (instr(strn(rs("STS FLAG")),"y") > 0) then
	' it's not a supressed record
  ...
  if strShow = "false" then
	...
	if strPubList <> "" then
		 strLink = " <a href='http://www.cabi.org/default.aspx?site=170&page=1016&pid=514' target='_blank'>Recent record: see Index of Fungi</a>"
	else
		 strLink = " <a href='http://www.cabi.org/default.aspx?site=170&page=1016&pid=514' target='_blank'>Recent record: see next part of Index of Fungi</a>"
	end if
  else
	 strLink = ""
  end if
	...
	if instr(strName," ") then strGenus = left(strName,instr(strName," ")-1)	
...
' it's a suppressed record [STS FLAG] = "y" AND [LAST FIVE YEARS FLAG] = "X"
strPubList = strn(rs("STS FLAG"))
if strPubList <> "y" then
	strPubList = strn(rs("PUBLISHED LIST REFERENCE"))
	if strPubList <> "" then
		 strLink = " <a href='http://www.cabi.org/default.aspx?site=170&page=1016&pid=514' target='_blank'>Recent record: see Index of Fungi</a>"
	else
	...
	response.write(DisplayName (rs, strShow) & ": " & strLink)
else
	response.write("EITHER no records were found with this IF record number OR the record is supressed<br><br>")
end if
...
' Kirk_net address
	if strA = "192" and strB = "168" and strC = "0" and strD = "12" then UserOK = "OK"
	if strA = "82" and strB = "43" and strC = "123" and strD = "182" then UserOK = "OK"
' LCR Christchurch proxy
	strLocalIP = request.servervariables("REMOTE_ADDR")
	if strLocalIP = "202.27.240.6" then UserOK = "OK"
' Kirk_net address
	strLocalIP = request.servervariables("LOCAL_ADDR")
	if strLocalIP = "82.43.123.182" then UserOK = "OK"
	strLocalIP = request.servervariables("LOCAL_ADDR")
	if strLocalIP = "192.168.0.12" then UserOK = "OK"
' Egham address
	strLocalIP = request.servervariables("REMOTE_ADDR")
	if strLocalIP = "81.106.220.1" then UserOK = "OK"
' Dave Minter address
	strLocalIP = request.servervariables("REMOTE_ADDR")
	if strLocalIP = "62.49.18.223" then UserOK = "OK"
end function
...
function DisplayName(rs, strShow)
	...
	if  strdata <> "" then
		strName = strName & "<b>" & encodeHtml(strData) & "</b>"
	end if	
...
sub DisplayLinkedData(rs, strShow)
	dim strData, strTax, strBSM, strData2, strLink, strSynonymy
	strData = ""
	strTax = ""
	if strShow = "True" then
		...
		if strData <> "" then
		' code to add link to homonym or valididation of name
			' check if there is a record number
			if instr(strData,"$") then
			' check if there is anything after year
				if instr(strData,"|") then
					strData1 = left(strData,instr(strData,"|")-2)
					strData2 = right(strData,len(strData)-instr(strData,"|"))
					intNumber = left(strData1,instr(strData1,"$")-1)
					strData = right(strData1,len(strData1)-instr(strData1,"$"))
				'check for second '$' for ' non ' record
					if instr(strData,"$") then
						strData3 = right(strData,len(strData)-instr(strData," non ")-4)
						strData = left(strData,instr(strData," non ")-1)
						intNumber2 = left(strData3,instr(strData3,"$")-1)
						strData4 = right(strData3,len(strData3)-instr(strData3,"$"))
					end if
				else
					intNumber = left(strData,instr(strData,"$")-1)
			...
			if left(strn(rs("EDITORIAL COMMENT")), 15) <> "Circumscription" then
			strCommentType = left(strn(rs("NOMENCLATURAL COMMENT")), 13)
				'if second link
				if strData4 <> "" then
					strLink = "<a href='NamesRecord.asp?RecordID=" & intNumber & "'>" & strData & "</a>"
					strLink2 = "<a href='NamesRecord.asp?RecordID=" & intNumber2 & "'>" & strData4 & "</a>"
					if instr(strCommentType,"illegit") then
						if strData2 <> "" then 
							response.write("<p><b>Editorial comment: </b><br>A homonym of <b>" & strLink & "</b> non " & strLink2 & ". " & strData2 & "</p>")
						else
							response.write("<p><b>Editorial comment: </b><br>A homonym of <b>" & strLink & "</b> non " & strLink2 & "</p>")
						end if
					elseif instr(strCommentType,"inval") then
						if strData2 <> "" then 
							response.write("<p><b>Editorial comment: </b><br>See <b>" & strLink & "</b>. " & strData2 & "</p>")
						else
							response.write("<p><b>Editorial comment: </b><br>See <b>" & strLink & "</b></p>")
						end if
					else
					...
					if instr(strCommentType,"illegit") then
						if strData2 <> "" then 
							response.write("<p><b>Editorial comment: </b><br>A homonym of <b>" & strLink & "</b>. " & strData2 & "</p>")
						else
							response.write("<p><b>Editorial comment: </b><br>A homonym of <b>" & strLink & "</b></p>")
						end if
					elseif instr(strCommentType,"inval") then
						if strData2 <> "" then 
							response.write("<p><b>Editorial comment: </b><br>See " & strLink & ". " & strData2 & "</p>")
						else
							response.write("<p><b>Editorial comment: </b><br>See " & strLink & "</p>")
						end if
					else
						response.write("<p><b>Editorial comment: </b><br>" & strData & "</p>")
					end if
				...
				response.write("<p><b>Editorial comment: </b><br>" & encodeHTML(strn(rs("EDITORIAL COMMENT"))) & "</p>")
			end if
		end if
		...
		if strData <> "" then
			if left(strData,13) = "Holotype IMI " then
				...
				if instr(strData," ") then strData = left(strData,instr(strData," "))
 				strLink = "http://www.herbimi.info/herbimi/specimen.htm?imi=" & strData
			...
			elseif left(strData,9) = "urn:lsid:" then
				...
				strPopup = ""
				strData = ""
			elseif instr(strData,"$") then strType = right(strData,len(strData)-instr(strData,"$"))
 				strData = left(strData,instr(strData,"$")-1)
				response.write("</p><b>Typification Details: <br><a href='NamesRecord.asp?RecordID=" & strData & "'>" & strType & "</a></b></p>")
				strPopup = ""
				strData = ""
			elseif left(strData,1) = "1" OR left(strData,1) = "2" OR left(strData,1) = "3" OR left(strData,1) = "4" OR left(strData,1) = "5" OR left(strData,1) = "6" OR left(strData,1) = "7" OR left(strData,1) = "8" OR left(strData,1) = "9" then
				response.write("<p><b>Typification Details: </b><br><a href='NamesRecord.asp?RecordID=" & strData & "'>Display IF name record for type</a></p>")
				strPopup = ""
				strData = ""
			else
		...
		if strData <> "" and instr(strData,"$") then
			if instr(strData," non ") then
				...
				response.write("<p><b>Competing homonym:</b><br>")
			...
			if strData <> "" then
				if instr(strData," non ") then
					...
					response.write("<p><b>Competing homonym:</b><br>" & strData & "</p>")
				end if
			end if
		end if
	...
	' added CODING TO GET RID OF IP numbers
	if left(rs("CORRECTION"),2) = "$N" then
		strPageImage = right(rs("CORRECTION"),len(rs("CORRECTION"))-2)
		strPageImage = replace(strPageImage,"10.0.5.10/librifungorum","www.librifungorum.org")
		strPageImage = replace(strPageImage,"82.43.123.182/librifungorum","www.librifungorum.org")
		strPageImage = strPageImage & chr(34) & chr(41)
		strPopup = "javascript:popUp(" & chr(34)
	...
	elseif left(rs("CORRECTION"),4) = "http" then
		strPageImage = rs("CORRECTION")
		strPageImage = strPageImage & chr(34) & chr(41)
		strPopup = "javascript:popUp(" & chr(34)
		strPopup = " <b><a href='" & strpopup & strPageImage & "'>" & "Page Image for Protologue</a></b>"
	end if
	...
	if strData <> "" then
		...
		elseif rtrim(left(rs("Phylum name"),instr(rs("Phylum name")," "))) = "Fossil" then
			...
			strData = strn(rs("subphylum name"))
			if strData <> "" then
				if strTax <> "" then
					strTax = strTax & ", " & strData
				else 
	...
	if strData <> "" then
		if clng(rs("RECORD NUMBER")) <> clng(rs("CURRENT NAME RECORD NUMBER")) _
			and clng(rs("CURRENT NAME RECORD NUMBER")) <> 0 then 
				response.write("<p><b>Species Fungorum current name:</b><br>")
				...
				strLinkType = 4
				strLink = "http://www.speciesfungorum.org/Names/SynSpecies.asp?RecordID=" & strData
				response.write("<a class='LinkColour" & strLinkType & "' href=" & strLink & " >" & strOut & "</a>")
		end if
	end if	
end sub
...
sub DisplaySyns(rs)
	...
	if left(strData,3) = "GSD" then
		...
		if strn(rs("CURRENT NAME RECORD NUMBER")) <> "" then 
			strGSD = strGSD & "<a href='http://www.speciesfungorum.org/GSD/GSDspecies.asp?RecordID=" & strn(rs("CURRENT NAME RECORD NUMBER")) & "'>Species Fungorum synonymy</a> "
		end if
...
sub CheckCurrent(rs)
	...
	elseif strIP = "127.0.0.1" then
	   ...
	   dbConn.open "FILEDSN=cabilink"
		  ...
		  & "WHERE (((IndexFungorum.[current name RECORD NUMBER])=" & protectSQL(rs("record number"),true) & "));" 
		...
		if rs2.RecordCount > 1 then
			...
			response.write("<a href='http://www.speciesfungorum.org/Names/SynSpecies.asp?RecordID=" & strn(rs("CURRENT NAME RECORD NUMBER")) & "'>Species Fungorum synonymy</a><br>")
		end if
	  	rs2.close
	  	set rs2 = nothing
	  	dbConn.close
...
sub CheckBasionyms(rs)
	...
	elseif strIP = "127.0.0.1" then
	   ...
	   dbConn.open "FILEDSN=cabilink"
   		...
   		strSQL ="SELECT IndexFungorum.[record number] from IndexFungorum " _
		  & "WHERE (((IndexFungorum.[Basionym RECORD NUMBER])= " & protectSQL(rs("record number"),true) & "));" 
  		Set rs2 = Server.CreateObject("ADODB.Recordset")
  		rs2.Open strSQL, dbConn, 3
		rs2.movelast
		if rs2.RecordCount > 1 then
