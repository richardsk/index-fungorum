...
<table width="100%" height="100%" class="mainbody">
  ...
  <tr>
	...
	if request.querystring.item("strFamily") <> "" then
...
sub ResList (rs)
	...
	elseif StrType = "G" then
		if right(rs("family name"),5) = "aceae" then
			if parenttaxon = "" then
				parenttaxon = "<a href='NamesHigher.asp?strFamily=" & rs("family name") & "'>" & rs("family name") & "</a>"
			end if
		end if
		if right(rs("order name"),4) = "ales" then
			if parenttaxon = "" then
	...
	'Code here for supraspecific results
...
sub DisplayRes
	...
	if strType = "N" then
		strSQL = "SELECT TOP 6000 IndexFungorum.[NAME OF FUNGUS], IndexFungorum.[MISAPPLICATION AUTHORS], IndexFungorum.AUTHORS, " _
			...
			& "WHERE p(((IndexFungorum.[NAME OF FUNGUS]) Like '" & protectSQL(strSearch, false) & "%')) " _
...
%>
            ...
            <form method="post" action="NamesHigher.asp" name="search">
              <table width="651" border="0" name="NamesHigher" height="60">
                <tr> 
