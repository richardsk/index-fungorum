...
<title>HerbIMI-Online - association search</title>
<!-- InstanceEndEditable -->
...
'	   strConn = "Provider=SQLOLEDB.1;Password=;Integrated security=SSPI;User ID=IUSR_WEBSERVER;Initial Catalog=herbIMI;Data Source=webserver"
	...
	if strSearch <> "" then
	   strSQL = "SELECT DISTINCT tblHerbIMI.AssociatedOrganism " _
			& "FROM tblHerbIMI " _
			& "WHERE (((tblHerbIMI.AssociatedOrganism) Is Not Null And " _
			& "(tblHerbIMI.AssociatedOrganism) Like '" & protectSQL(strSearch, false) & "%')) " _
			& "ORDER BY tblHerbIMI.AssociatedOrganism;"
	else
	exit sub
	end if
...
sub ResList(RS)
	dim strLink, strOut
		response.write("<p></p><b>Associated organisms: " & strn(rs("AssociatedOrganism")) & "</b><p></p>") 
		do while not RS.bof and not RS.eof
		strText = ""
		strText = strn(rs("AssociatedOrganism"))
		if strText <> "" then 
		   strLink = "DisplayResults.asp?strAssociation=" & server.urlencode(rs("AssociatedOrganism")) 
		   response.write("<a href=" & strLink &  " >" & strText & "</a><br>")
		end if
		RS.MoveNext
		loop
end sub
...
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Main" -->
            <form method="post" action="Association.asp" name="search">
              <table width="359" border="1" name="Name" height="60" bgcolor="#99FF99" bordercolor="#000000">
                <tr align="left" bordercolor="#99FF99" bgcolor="#99FF99"> 
                  <td width="163" height="15" bordercolor="#99FF99"><b><font face="Verdana, Arial, Helvetica, sans-serif">Associated 
                    organism:</font></b></td>
                ...
                <tr align="left" bordercolor="#99FF99" bgcolor="#99FF99"> 
                  ...
                  <td width="180" height="3"> 
                    <input type="submit" name="submit" value="Search for association">
				  </td>
                </tr>
              </table>
			  </form>
