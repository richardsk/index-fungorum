...
'	strItemID = session("strItemID")
	...
	if request.querystring.item("ItemID") <> "" then
	  	session("strItemType") = ""
...
sub GetNav
	select case request.querystring("pg")
		case "-1"
			clng(intPageNumber) = clng(intPageNumber) - 1
		case "+1"
			clng(intPageNumber) = clng(intPageNumber) + 1
		case else
...
sub DisplayRes
	...
	if strItemType <> "" or strItemID <> "" then
...
'			strSQL = "SELECT tblItems.*, tblImages.* " _
'				& "FROM tblItems INNER JOIN tblImages ON tblItems.ID = tblImages.ItemFK " _
'				& "WHERE (((tblImages.ItemFK) = " & strItemID & ")) " _
...
'		end if
		...
		set dbConn = nothing
			...
			response.write(strItemType & "<br>")
			response.write(strItemID & "<br>")

		response.write("<p>Type of Resource:</p>")
		response.write("<p><a href='Search.asp?ItemType=B'>Books</a></p>")
		response.write("<p><a href='Search.asp?ItemType=J'>Journals</a></p>")
		response.write("<p><a href='Search.asp?ItemType=T'>Thesauri</a></p>")
		response.write("<p><a href='Search.asp?ItemType=I'>Indexes</a></p>")
		response.write("<p><a href='Search.asp?ItemType=M'>Miscellaneous</a><br>")
		response.write("</p>")
	end if
end sub
